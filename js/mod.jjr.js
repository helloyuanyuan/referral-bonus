...
KISSY.use('node,io', function(S, Node, IO) {
	...
	submit_broker.on('click', function() {
		...
		IO.post('index.php?c=register&f=checkphone&phone='+pv+'',function(data) {
			if (data == 0) {
				IO.post('index.php?c=register&f=setok&name='+nv+'&phone='+pv+'&password='+psw+'&job='+prv+'&company='+prCompany+'',function(data) {
					if (data == 1) {
						alert("注册成功");
    ...
    J_login.on('click', function() {
        ...
        IO.post('index.php?c=login&f=ok&phone='+pv+'&password='+ups+'', function(data) {
            if (data == 1) {
               window.location.href = '/index.php?c=home';
            } else {
                alert(data);
	...
	saveCard.on('click', function() {
		...
		IO.post('index.php?c=usercp&f=savebank&bankAccount='+account+'&cardCode='+num+'&bankName='+name+'', function(data) {
			if (data == 1) {
				window.location.href = '/index.php?c=commission';
			} else {
				alert(data);
			}
		});
