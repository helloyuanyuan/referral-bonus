...
class commission_m extends Model
{
	...
	{
		$sql = "SELECT * FROM ".$this->db->prefix."commission ";
		if($condition)
		{
			$sql .= " WHERE ".$condition;
		}
	...
	{
		$sql = "SELECT count(id) FROM ".$this->db->prefix."commission ";
		if($condition)
		{
			$sql .= " WHERE ".$condition;
		}
	...
	{
		$sql = "UPDATE ".$this->db->prefix."commission SET status='".$status."' WHERE id='".$id."'";
		return $this->db->query($sql);
	}
	function status($id,$status=0)
	{
		$sql = "UPDATE ".$this->db->prefix."commission SET status='".$status."'  WHERE id IN(".$id.")";
		return $this->db->query($sql);
	}

	function del($id)
	{
		$sql = "DELETE FROM ".$this->db->prefix."commission WHERE id='".$id."'";
		return $this->db->query($sql);
	}
	function pl_del($id)
	{
		$sql = "DELETE FROM ".$this->db->prefix."commission WHERE id IN(".$id.")";
		return $this->db->query($sql);
	}
}
?>